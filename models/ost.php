...
class Ost extends MainModel
{
    ...
    // résultat de la recherche avec le filtre
    public function infoPageOST($firstsOst, $numberResultsPage, $searchOstList, $ostFilter = ['album'])
    {
        ...
        $infoPage = $this->pdo->prepare(
            'SELECT `op`.`id`, `ost`.`id` , `ost`.`name` AS `ostName`, `album`, `date` , `path` , `title` , `alt` , `category`.`id` AS `categoryID`, `category`.`name` AS `categoryName` , `categorizedby`.`id_OST` , `composerlist`.`id_OST` , `lastname` , `firstname` , `id_Composer`
            FROM `ost`
            LEFT JOIN `ostpicture` AS `op`
            ON `id_OSTPicture` = `op`.`id`
            LEFT JOIN `categorizedby`
    ...
    public function searchOst($searchOstList)
    {
        $pdoStatment = $this->pdo->prepare(
           'SELECT 
                `ost`.`name` AS `ostName`, `album`, `composerlist`.`id_OST` , `lastname` , `firstname`, `ost`.`id`,  `path` , `title` , `alt`
            FROM 
                `ost`
                LEFT JOIN `composerlist` ON `ost`.`id` = `composerlist`.`id_OST`
                LEFT JOIN `composer` ON `composer`.`id` = `id_Composer` 
                LEFT JOIN `ostpicture` AS `op` ON `id_OSTPicture` = `op`.`id`
            WHERE 
